...
interface BaseProps {
  ...
  encontrarMejorTransportista: (destino: string, origen: string, tipoCargamento: string) => Transportista | undefined;
}
...
function Base({ transportistas, encontrarMejorTransportista }: BaseProps) {
  ...
  const handleSubmit = (origen: string, destino: string, tipoCargamento: string) => {
    const mejorTransportista = encontrarMejorTransportista(destino, origen, tipoCargamento);
    if (mejorTransportista) {
      const { Transportista, "Experiencia (años)": experiencia, "Días en Carretera": diasEnCarretera, "Porcentaje de Carga": porcentajeCarga, Descripcion } = mejorTransportista.fields;
      setRecomendacion(`
      ...
      setRecomendacion('<p class="text-red-500">No se encontró un transportista adecuado para tus requerimientos.</p>');
    }
  };
  ...
  return (
    <main className="h-[88%] grid grid-cols-2 gap-4 p-4">
      <div className="bg-slate-500 rounded-md p-4">
        <h2 className="text-2xl text-white mb-4">Información del Transporte</h2>
        {/* Aquí puedes agregar más contenido informativo */}
      </div>
      <div className="bg-slate-500 rounded-md">
        <FormularioTransporte onSubmit={handleSubmit} transportistas={transportistas} />
      </div>
      <div className="col-span-2 bg-slate-500 rounded-md p-4">
        <h2 className="text-2xl text-white mb-4">Recomendación del Transportista</h2>
        {recomendacion ? (
          <div className="bg-white p-4 rounded-md" dangerouslySetInnerHTML={{ __html: recomendacion }} />
        ) : (
          <p className="text-white">Aquí aparecerá la recomendación del mejor transportista.</p>
        )}
      </div>
    </main>
  );
